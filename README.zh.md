...
> **从 ReAct 到 MindAct。** [ReAct](https://arxiv.org/abs/2210.03629)（Reasoning + Acting）是当前大多数 AI Agent 的基础范式——将思维链推理与工具调用交替进行。MindAct 更进一步：在 Agent 行动前，它先检查自己知道什么、不知道什么、以及置信度有多高。
...
<p align="center">
  <img src="assets/hero_screenshot.png" alt="MindAct 桌面 — Brain Graph × Claude Code 终端" width="100%" style="border-radius: 10px;">
  <br>
...
**[ReAct](https://arxiv.org/abs/2210.03629)**（Reasoning + Acting，Yao et al. 2022）是当前大多数 AI Agent 背后的范式：将思维链*推理*与工具*行动*交替循环执行，使模型能够规划、执行、观察并调整。Claude Code 等 Agent 的底层运作方式就是如此——逐步思考，然后调用工具，再根据结果进行推理。
...
├── setup.sh                   # 一键安装：编译 CLI + 安装依赖
├── restart.sh                 # 开发启动器：构建客户端 + 启动服务（可选 Electron）
...
│       │   ├── KBPanel.tsx         # 决策依据面板
│       │   ├── SkillsExplorer.tsx  # 专家能力面板 — 文件夹加载 UI
│       │   ├── FileExplorer.tsx    # 项目文件面板
...
**只用浏览器（不要 Electron / 不要 X11）：** 在无桌面或 SSH 服务器上可跳过桌面壳，用网页访问：
...
**若缺少 `cli/rust/`**（克隆时未拉取子模块），先拉取 Rust CLI 再执行安装：
...
子模块仓库：[physmind-cli-rust](https://github.com/KeploreAI-Lab/physmind-cli-rust)。
...
5. 在 **Linux** 上，若包内无 linux 预编译，则从源码编译 **`node-pty`**（应用内终端）
...
### 开发：已有 3001 服务时只跑前端热更新

在仓库**根目录**执行 `bun run dev` 会同时启动 **3001**（Bun）和 **5173**（Vite）。若 3001 已被占用（例如已执行 `./restart.sh` 或 `bun run server.ts`），会报 `EADDRINUSE`，因为不能绑定第二个 3001。
...
浏览器打开 [http://localhost:5173](http://localhost:5173)。Vite 会把 `/api` 代理到 `http://localhost:3001`（见 `client/vite.config.ts`）。终端 WebSocket 仍直连 3001。
...
### Electron：`libatk-1.0.so.0` 等缺少 `.so`（Linux）

精简服务器常未安装 Electron 所需的 GTK/Chromium 系统库。**`./setup.sh`** 与 **`./restart.sh`** 会执行 `scripts/check-linux-electron.sh`，在终端打印 **`ldd` 缺失项** 与 **`apt-get install` 示例**，若检查失败 **`restart.sh` 会跳过启动 Electron**，避免静默失败。

- **日志：** `/tmp/mindact-electron.log`（尝试启动 Electron 时）
- **手动检查：** `bash scripts/check-linux-electron.sh`（脚本会根据系统选用 **`libasound2t64`** / **`libgtk-3-0t64`**（Ubuntu 24.04+）或旧包名 **`libasound2`** / **`libgtk-3-0`**）。
- **不用桌面壳：** **`MINDACT_SKIP_ELECTRON=1 ./restart.sh`**，或保持 **3001** 并执行 **`cd client && bun run dev`** → **http://localhost:5173**（见上文 **开发：已有 3001 服务时只跑前端热更新**）。

### Electron：`chrome-sandbox` / SUID 沙箱（Linux）

若 `node_modules/electron/dist/chrome-sandbox` 不是 **root 拥有** 且权限 **4755**（setuid），Chromium 会直接退出并在日志中报 fatal。

- **`./restart.sh`** 会检测并在需要时设置 **`MINDACT_ELECTRON_NO_SANDBOX=1`**，以 **`--no-sandbox`** 启动 Electron（本地开发可接受；隔离弱于完整沙箱）。
- **更严格（可选）：** `sudo chown root:root node_modules/electron/dist/chrome-sandbox && sudo chmod 4755 node_modules/electron/dist/chrome-sandbox`（重装 `electron` 后需重做）。
- **自行启动时：** 在启动 Electron 前导出 **`MINDACT_ELECTRON_NO_SANDBOX=1`**。

### Electron：`Missing X server` / 无 `$DISPLAY`（Linux）

Electron 需要**图形显示**。在 SSH 或无头环境请用**浏览器**：**`MINDACT_SKIP_ELECTRON=1 ./restart.sh`**，再 **`cd client && bun run dev`** → **http://localhost:5173**。

### 终端：立刻出现「Process exited / 点击 Restart」

应用内终端由 **Node.js** 子进程加载 **`node-pty`**。若原生模块缺失，该进程在向服务端输出 JSON 前就退出；**`/tmp/mindact-server.log`** 会出现 **`[PTY] worker done, sent 0 msgs`**（若 Node 打了错误，还会有 **`[PTY] worker stderr:`** 行）。

- **修复：** 在仓库根目录执行 **`cd node_modules/node-pty && npx --yes node-gyp@10 rebuild`**（先安装 **`build-essential`** 或本发行版 C++ 工具链）。**`./setup.sh`** 在 Linux 上会在 **`bun install`** 后若 **`require('node-pty')`** 失败则自动尝试编译。
- **原因：** 许多 **`node-pty`** 包只带 **darwin/win32** 预编译，**Linux** 需在本地生成 **`pty.node`**。
...
| 变量名 | 是否必需 | 说明 |
|---|---|---|
| `KPLR_KEY` | 是 | KeploreAI key（`kplr-...`）。在 **设置** 中填写 — 保存到 `~/.config/physmind/credentials` |
| `CLAUDE_BIN` | 否 | 覆盖 CLI 可执行文件路径（默认从 **`PATH`** 自动检测） |
| `MINDACT_ELECTRON_NO_SANDBOX` | 否 | Linux 上设为 `1` 可在 `chrome-sandbox` 非 root setuid 时用 `--no-sandbox` 启动 Electron；需要时 **`restart.sh` 会自动设置**。 |
| `MINDACT_SKIP_ELECTRON` | 否 | 设为 `1` 时 **`./restart.sh`** 不启动 Electron（浏览器：**3001** + **`cd client && bun run dev`** → **5173**）。 |
